...
// If course_code is provided, get course_id from courses table
if ($courseCode && !$courseId) {
    try {
        $courseSql = "SELECT course_id FROM courses WHERE course_code = :course_code";
        ...
        if ($course) {
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Course not found with code: ' . $courseCode
            ]);
            exit;
        }
    } catch (PDOException $e) {
        echo json_encode([
            'success' => false,
            'message' => 'Error finding course: ' . $e->getMessage()
        ]);
        exit;
    }
}

// Default course_id if neither is provided
if (!$courseId) {
    $courseId = 1;
}
