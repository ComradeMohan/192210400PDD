...
$db = "univault_db"; // <-- Confirmed DB name from your schema
...
// NEW: File filter function for the File Editor page
function filterFiles() {
    const searchInput = document.getElementById('file_search');
    const filterSelect = document.getElementById('file_type_filter');
    const table = document.getElementById('file_list_table');
    
    if (!table || !searchInput || !filterSelect) return;
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        const fileType = row.getAttribute('data-file-type') || 'other';

        // 1. Check if file name matches search term
        const nameMatch = fileName.toLowerCase().includes(searchTerm);
        
        // 2. Check if file type matches filter
        let typeMatch = false;
        if (filter === 'all' || filter === fileType) {
            typeMatch = true;
        }
...
function include_file_editor() {
    ...
    <h3 style="margin-top: 20px;">File Browser</h3>
    <div style="display: flex; gap: 20px; margin-bottom: 20px;">
        <div class="form-group" style="flex: 1;">
            <label for="file_search">Search Filename:</label>
            <input type="text" id="file_search" onkeyup="filterFiles()" placeholder="Search by filename...">
        </div>
        <div class="form-group" style="flex: 1;">
            <label for="file_type_filter">Filter by Type:</label>
            <select id="file_type_filter" onchange="filterFiles()">
                <option value="all">All Files</option>
                <option value="php">PHP (.php)</option>
                <option value="html">HTML (.html)</option>
                <option value="css">CSS (.css)</option>
                <option value="js">JavaScript (.js)</option>
                <option value="txt">Text/Config (.txt, .md, .log, .json etc.)</option>
                <option value="other">Other/No Extension</option>
            </select>
        </div>
    </div>
    ...
    <div class="table-container" style="margin-top: 20px;">
        <table id="file_list_table"> <!-- Added ID for JS filtering -->
            <thead>
                <tr>
                    <th>Filename</th>
                    <th>Size</th>
            ...
            <tbody>
                <?php foreach ($current_dir_files as $file): 
                    ...
                    // Determine file extension and type for filtering
                    $ext = pathinfo($file, PATHINFO_EXTENSION);
                    $file_type = 'other';
                    if (empty($ext) && strpos($file, '.') === false) {
                        $file_type = 'other';
                    } elseif (in_array($ext, ['php'])) {
                        $file_type = 'php';
                        ...
                        $file_type = 'js';
                    } elseif (in_array($ext, ['txt', 'md', 'json', 'log', 'ini', 'xml'])) {
                        $file_type = 'txt';
                    }
                ?>
                <tr data-file-name="<?php echo htmlspecialchars($file); ?>" data-file-type="<?php echo $file_type; ?>"> <!-- Added data attributes for JS filtering -->
                    <td>
