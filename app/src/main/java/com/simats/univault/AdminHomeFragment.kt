...
class AdminHomeFragment : Fragment() {
    ...
    private fun fetchCollegeIdByName(collegeName: String, onResult: (String?) -> Unit) {
        val url = "http://192.168.137.229/univault/get_college_id.php" // Replace IP if testing on device
    ...
    private fun fetchAdminDetails(adminId: String) {
        val url = "http://192.168.137.229/univault/getAdminDetails.php?admin_id=$adminId"
    ...
    private fun fetchLatestNotice(college: String) {
        val url = "http://192.168.137.229/univault/get_latest_notice.php?college=$college"
        val request = JsonObjectRequest(Request.Method.GET, url, null,
            { response ->
                if (!isAdded || _binding == null) return@JsonObjectRequest
                val safeBinding = _binding!!
    ...
    private fun fetchFeedbacks() {
        ...
        val url = "http://192.168.137.229/univault/get_feedbacks.php?college=$college"
    ...
    private fun getNextFacultyId(callback: (String) -> Unit) {
        ...
        val url = "http://192.168.137.229/univault/getNextFacultyId.php?college=${college.replace(" ", "%20")}"
    ...
    private fun postFacultyToServer(
        ...
        val url = "http://192.168.137.229/univault/faculty_register.php"
